...
_DEFAULT_TESTNET = "https://api.hyperliquid-testnet.xyz"
_DEFAULT_MAINNET = "https://api.hyperliquid.xyz"
API_BASE_URL = os.getenv("HYPERLIQUID_API_BASE_URL", _DEFAULT_TESTNET).rstrip("/")
PUBLIC_BASE_URL = os.getenv("HYPERLIQUID_PUBLIC_BASE_URL", _DEFAULT_MAINNET).rstrip("/")
...
async def get_perp_assets() -> Optional[Set[str]]:
    ...
    try:
        info = Info(API_BASE_URL, skip_ws=True)
        meta = info.meta()
        perp_assets = set()
